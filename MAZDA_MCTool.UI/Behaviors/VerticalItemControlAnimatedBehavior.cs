...
using MAZDA_MCTool.UI.Utilites;

namespace MAZDA_MCTool.UI.Behaviors;

public class VerticalItemControlAnimatedBehavior : StyledElementBehavior<ItemsControl>
{
    private ScrollViewer? _scrollViewer = null;
    /// <summary>
    /// ScrollStepSize DirectProperty definition
    /// Modified from https://github.com/Egolds/Avalonia.Xaml.Interactions.Animated
    /// </summary>
    ...
    {
        _scrollViewer = AssociatedObject!.GetVisualDescendants().FirstOrDefault(v => v.GetType() == typeof(ScrollViewer)) as ScrollViewer;
        ...
        _scrollViewer!.SetValue(ScrollChangeSizeProperty, ChangeSize.Line);

        scp = _scrollViewer?.Presenter as ScrollContentPresenter;

        _scrollViewer!.Loaded -= AssociatedObject_Loaded;
    }
    ...
    private Vector SnapOffset(Vector offset, Vector direction = default, bool snapToNext = false)
    {
        ...
        if (scrollable is IScrollSnapPointsInfo scrollSnapPointsInfo)
        {
            ...
            {
                _verticalSnapPoints = scrollSnapPointsInfo.GetIrregularSnapPoints(Avalonia.Layout.Orientation.Vertical, scp!.VerticalSnapPointsAlignment);
            }
            else
            {
                _verticalSnapPoints = new List<double>();
                _verticalSnapPoint = scrollSnapPointsInfo.GetRegularSnapPoints(Avalonia.Layout.Orientation.Vertical, scp!.VerticalSnapPointsAlignment, out _verticalSnapPointOffset);
            }
        }
    ...
    private void OnPointerWheelChanged(object? sender, PointerWheelEventArgs e)
    {
        ...
        if (delta.Y > 0) // Scroll up
        {
            ...
            {
                AnimateScroll(-_scrollViewer!.Bounds.Height);
            }
        }
        else // Scroll down
        {
    ...
    private void AnimateScroll(double delta)
    {
        ...
        if (_isAnimating)
        {
            ...
            _targetOffset += delta;
            _targetOffset = Math.Clamp(_targetOffset, 0, _scrollViewer!.Extent.Height - _scrollViewer!.Bounds.Height);
